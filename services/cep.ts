export class CepError extends Error {
  constructor(public readonly code: "NOT_FOUND" | "GEOCODE_FAILED" | "NETWORK_ERROR") {
    super(code);
    this.name = "CepError";
  }
}
...
export async function fetchCoordinatesFromCep(cep: string): Promise<CepCoordinates> {
  ...
  return { lat: parseFloat(lat), lon: parseFloat(lon), cityName: `${localidade}, ${uf}` };
}
